...
#### 🔵 ` 문제 ` 
BaseEntity를 상속 받는데 BaseEntity와 필드명이 DB 컬럼명과 일치하지 않아 찾을 수 없다는 에러 발생  
#### 🟢 ` 해결 방법 `  
컬럼 명 일치로 해결 완료
...
#### 🔵 ` 문제 `
의존성 도입 도중 repository 메소드에 제대로 된 필드 값이 들어오지 않아 발생
#### 🟢 ` 해결 방법 `
JpaRepository에 페이지네이션으로 구현했으나 파라미터에 pageable을 넣지 않아 발생했다. pageable을 파라미터로 넣어주어 해결
...
<p style="margin: 2px 0;">이건 첫 번째 줄입니다.</p>
<p style="margin: 2px 0;">이건 두 번째 줄입니다.</p>
