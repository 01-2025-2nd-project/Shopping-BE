...
- ` 파티 참여 ` :  토큰을 받아 유저 확인, partyId를 받아 파티 존재하는지 확인, 유저에 money가 모집하는 파티 상품의 가격보다 많은지 확인 후 파티 참여 진행
- ` 파티 나가기 ` :  토큰을 받아 유저 확인, partyId를 받아 파티 존재하는지 확인, 파티 나가기 후 유저 money 복구
...
*협업* 
```
프로젝트를 처음 진행하는 과정에서 백엔드와 프론트간 협업 과정이 익숙치 않아 적응하는 시간이 오래 걸렸지만 주기적인 회의를 통해 코드를 리뷰하고 소통함으로써 성공적으로 프로젝트를 마칠 수 있었습니다. 이를 통해 협업에 대한 이해도가 상승하였고 다음 프로젝트에서는 협업을 하면서 겪었던 어려움을 잘 해결할 수 있을 것으로 기대됩니다 🐰
```

*설계의 중요성* 
```
프론트와 백엔드 간의 회의를 통하여 api 명세서를 설계하면서 처음인 부분들이 많아 개발 중간중간에 필드명 또는 추가될 필드 받을 필드 등이 여러번 수정되어 개발에 불편이 있었습니다. 이번 프로젝트를 진행함으로써 초반 설계를 어떻게 해야할지 알게되어 다음 프로젝트에서는 수월하게 진행될 거 같습니다😺
```

*기록* 
```
 개발을 진행하면서 마주한 많은 버그를 노션에 기록하고 공유함으로써 같은 버그를 마주하더라도 금방 해결할 수 있어 유용하다는 것을 알게되었습니다. 앞으로도 기록을 꾸준히 하여 이전에 겪은 버그 또는 다른 팀원들에게서 발생했던 버그들을 효율적으로 해결해 나갈 수 있을 거 같습니다.🐰😺
```
